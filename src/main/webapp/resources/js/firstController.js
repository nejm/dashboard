myApp.controller('FirstExampleController', function ($rootScope, $scope, $http, $localStorage, $uibModal, $compile) {
    ...
    /*$http.get('d.json').then(function(response){
    ...
    $scope.addService = function (service, res) {
        ...
        $('#' + uid)
                .append($compile("<li class='list-group-item'>" +
                        service.name + "<a class='pull-right' ng-click=drawService('" + service.id + "')>" +
                        "<span class='glyphicon glyphicon-plus'></span></a></li>")($scope));
    ...
    $scope.getResult = function (res, res2) {
        $scope.statAttributes = [];
        let a = false;
        let joined = false;
        ...
        if ($scope.query.stat == 'Tableau') {
            $scope.generateTable(res);
        } else if ($scope.query.stat == 'Bar') {
            $scope.generateBar(res, $scope.query.x, $scope.query.y);
        }
        //})
        //console.log(res);
    }

    $scope.generateTable = function (res) {
    ...
    $scope.generateBar = function (res, x, y) {
        $scope.labelsBar = [];
        $scope.dataBar = [];
    ...
    $scope.openModal = function (name, id) {
        ...
        $scope.modalInstance = $uibModal.open({
            templateUrl: 'resources/partials/' + name + '.html',
            scope: $scope
        });
    };
    ...
    $scope.projectName="";
    $scope.exporter = function (projectName) {
        var result = {};
        result.users = $scope.users;
        result.profiles = $scope.profiles;
        result.name = projectName;
        result.stateObjects = $scope.stateObjects;
        result.links = $scope.links;
    ...
    $scope.saveState = function () {
        $scope.modalInstance = $uibModal.open({
            templateUrl: 'resources/partials/saveState.html',
            scope: $scope
        });
    }
    ...
    $scope.importer = function () {
        $scope.modalInstance = $uibModal.open({
            templateUrl: 'resources/partials/importer.html',
            scope: $scope
        });
    }
    $scope.new = 20;
